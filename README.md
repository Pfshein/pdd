...
pdd diff DEMO-1
pdd report DEMO-1
pdd reap            # dry-run чистки зависших джоб и протухших lease очереди
pdd cleanup DEMO-1
```
...
Если воркер упал, не сняв lease, протухшую запись подберёт `pdd reap`: если state джоба уже
терминальный — очередь принимает финальный статус, иначе джоб возвращается в `queued`.
