...
Джоб можно поставить в durable-очередь, не запуская сразу (исполнение очереди воркером —
следующая карточка PDD-27):
...
id поставленного джоба; повторная постановка ещё активного джоба отклоняется.
