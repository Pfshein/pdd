...
Тесты: **120 passed** (`python -m pytest -q`). Образ `pdd-sandbox:latest`, сеть `pdd-internal`,
...
### 2. PDD-12 — hardening песочницы  *(в работе)*
...
**Рекомендуемый порядок:** 2 (hardening) → 7 (логи) → 4 (Jira) →
...
git switch -c PDD-12-sandbox-hardening` и реализовать задачу 2.
