...
Тесты: **117 passed** (`python -m pytest -q`). Образ `pdd-sandbox:latest`, сеть `pdd-internal`,
...
### 3. PDD-16 — техдолг-гигиена: reaper + чистка артефактов на старте  *(в работе)*
...
**Рекомендуемый порядок:** 3 (гигиена) → 2 (hardening) → 7 (логи) → 4 (Jira) →
...
git switch -c PDD-16-job-hygiene` и реализовать задачу 3.
